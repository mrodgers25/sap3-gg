...
module Admin
  class ReportsController < Admin::BaseAdminController
    include SendGrid
    ...
    def export_userlisting
      ...
      output = CSV.generate do |writer|
        writer << %w[id first_name last_name role email city_preference created_at confirmation_sent_at
    ...
    def export_all
      ...
      CSV.open(file_o, 'w') do |writer|
        writer << %w[Id UserId Url Created]
      ...
      mail = SendGrid::Mail.new do |m|
        m.to = logged_in_user_email.to_s
      ...
      mail.add_attachment(file_s.to_s)
      mail.add_attachment('tmp', 'story_listing.csv')
      mail.add_attachment(file_u.to_s)
      mail.add_attachment(file_a.to_s)
      mail.add_attachment(file_o.to_s)

      # #Part of original code
      puts client.send(mail)

      redirect_to :back,
                  notice: "Exports created and sent. They should arrive in about 10 minutes at #{logged_in_user_email}"
    end
    ...
    ")
      ...
      csv = CSV.generate do |csv_data|
        csv_data << %w[id story_id story_type story_title activity_type details pinned pinned_action posted_at
                       cleared_at created_at time_pinned time_queued time_posted]
