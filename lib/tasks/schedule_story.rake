  ...
  task check_pub_time: :environment do
    ...
    if stories_per_day > 0
      ...
      9.times do
        ...
        last_story_published_at = Story.order('sap_publish_date DESC').where.not(sap_publish_date: nil).pluck('sap_publish_date').first
        ...
        next_story_to_publish = Story.order('release_seq', 'created_at').where(sap_publish_date: nil,
                                                                               story_complete: 'true').pluck('id').first
        # puts "next_story_to_publish ---> #{next_story_to_publish}"

        if Code.where("code_key = 'NEXT_STORY_PUB_DATETIME'").present?
          next_story_pub_datetime_arr = Code.where("code_key = 'NEXT_STORY_PUB_DATETIME'").pluck('id',
                                                                                                 'code_value').first
