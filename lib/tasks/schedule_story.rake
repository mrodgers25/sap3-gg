  ...
  task check_pub_time: :environment do
    ...
    unless next_story_to_publish.nil?
      if time_left_before_pub ||= 0 <= 0
        ...
        puts "updating code table 'NEXT_STORY_TO_PUBLISH' using code id ---> #{next_story_to_publish} with #{Time.now + 3600).in_time_zone("Pacific Time (US & Canada)").strftime("%Y-%m-%dT%H:%M:%S%z"}"
