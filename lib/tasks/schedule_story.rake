...
namespace :schedule_story do
  ...
  task check_pub_time: :environment do
    ...
    if stories_per_day.positive?

      include Rails.application.routes.url_helpers
      9.times do
        start_loop = Time.zone.now
        ...
        next_story_to_publish = Story.order('release_seq', 'created_at').where(sap_publish_date: nil,
                                                                               story_complete: 'true').pick('id')
        # puts "next_story_to_publish ---> #{next_story_to_publish}"

        if Code.where("code_key = 'NEXT_STORY_PUB_DATETIME'").present?
          next_story_pub_datetime_arr = Code.where("code_key = 'NEXT_STORY_PUB_DATETIME'").pick('id',
                                                                                                'code_value')
        ...
        elsif (time_left_before_pub ||= 0) <= 0
          Story.find(next_story_to_publish).update(sap_publish_date: Time.zone.now)
          Code.find(next_story_pub_datetime_arr[0]).update(code_value: (Time.zone.now + stories_every_x_secs).in_time_zone('Pacific Time (US & Canada)').strftime('%Y-%m-%dT%H:%M:%S%z'))
        ...
        end_loop = Time.zone.now
        # puts "Loop timing: #{end_loop - start_loop}"
        sleep 60
      end
    end
  end
