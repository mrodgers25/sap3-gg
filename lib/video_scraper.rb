...
class VideoScraper
  ...
  attr_reader :title, :meta_desc, :meta_type, :meta_keywords, :meta_author, :clean_text, :month, :day, :year,
              :page_imgs, :link_creator, :link_channel_id, :link_image, :page_imgs
  ...
  def scrape!(scrape_url)
    ...
    # creator
    @link_channel_id = doc.css("link[itemprop='url']")&.map do |element|
                         element['href']
                       end&.select { |x| x.include? 'channel' }&.first&.split('/')&.last
    # Link
    ...
    begin
      ...
      if image_size_array[0].to_i > 199
        @page_imgs << { 'src_url' => @link_image, 'alt_text' => 'Thumbnail Image',
                        'image_width' => image_size_array[0], 'image_height' => image_size_array[1] }
      end
    rescue StandardError
      @page_imgs << { 'src_url' => @link_image, 'alt_text' => 'Thumbnail Image', 'image_width' => 1080,
                      'image_height' => 720 }
    end
